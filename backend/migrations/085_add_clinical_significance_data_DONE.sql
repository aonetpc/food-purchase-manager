-- ============================================
-- 体检项目体检意义（clinical_significance）数据
-- 迁移脚本: 085_add_clinical_significance_data.sql
-- ============================================
...
UPDATE booking_checkup_items SET clinical_significance = '心脑血管疾病独立危险因素，评估血栓风险' WHERE name = '同型半胱氨酸(HCY)' AND category = '化验' AND status != 0;
UPDATE booking_checkup_items SET clinical_significance = '反映肝脏合成与营养状况，敏感评估肝功能' WHERE name = '前白蛋白(PA)' AND category = '化验' AND status != 0;
...
UPDATE booking_checkup_items SET clinical_significance = '注意：此项可能命名有误，AFP定性应为肝癌筛查' WHERE name = '甲胎蛋白定性（CEA）' AND category = '化验' AND status != 0;
...
UPDATE booking_checkup_items SET clinical_significance = '定性检测癌胚抗原，辅助消化道肿瘤筛查' WHERE name = '癌胚抗原定性（CEA）' AND category = '化验' AND status != 0;
