...
// ================================================
// H5端接口（独立token认证，免登录）
// 【路由顺序优先级】字面路径 /h5/* 必须在参数路由 /:id* 之前注册，
//   否则 GET /stock-takes/h5/meta 会被 GET /stock-takes/:id 先匹配，执行 requireAuth 直接 401，
//   永远到不了 H5 handler。（Express 路由按注册顺序匹配）
// 同时所有 PC 端 /:id* 路由仍保留 next('route') 兜底（见各 PC 路由首行）作为双保险。
// ================================================
...
router.post('/h5/review', requireStockTakeToken, async (req, res) => {
  ...
  try {
    ...
    // 1. 更新盘点单状态
    const [diffItems] = await pool.query(`
...
router.get('/:id', requireAuth, async (req, res, next) => {
  // 🔁 路由顺序兜底：H5 GET /stock-takes/h5/meta 的 :id='h5' 会先命中此 PC 路由，
  //    requireAuth 对未登录 H5 场景直接 401，导致永远到不了后面的 GET /h5/meta。
...
router.put('/:id', requireAuth, async (req, res, next) => {
  // 🔁 路由顺序兜底：H5 PUT /stock-takes/h5/save 的 :id='h5' 会先命中此 PC 路由
  if (req.params.id === 'h5') return next('route');
  const conn = await pool.getConnection();
  try {
    const { id } = req.params;
...
router.delete('/:id', requireAuth, async (req, res, next) => {
  // 🔁 路由顺序兜底：H5 暂无 DELETE /h5/*，防御性兜底，避免未来扩展时再踩坑
  if (req.params.id === 'h5') return next('route');
  try {
    const { id } = req.params;
    const [takeRows] = await pool.query('SELECT * FROM stock_takes WHERE id = ?', [id]);
 ...
 */
router.get('/:id/review-init', requireAuth, async (req, res, next) => {
  // 🔁 路由顺序兜底：三段路径理论不冲突，防御性兜底以防 /h5/xxx/review-init 未来扩展
  if (req.params.id === 'h5') return next('route');
  try {
    const { id } = req.params;
...
router.post('/:id/notify', requireAuth, async (req, res, next) => {
  // 🔁 路由顺序兜底：防御性兜底，避免 H5 /h5/* 路由未来扩展两段路径时被误匹配
  if (req.params.id === 'h5') return next('route');
  try {
    const { id } = req.params;
