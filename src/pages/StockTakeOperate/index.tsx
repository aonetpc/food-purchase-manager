...
export default function StockTakeOperate() {
  ...
  // ---- 派生数据 ----
  const editable = meta ? ['draft', 'returned'].includes(meta.status) : false;
  ...
  const handleSubmit = async () => {
    ...
    if (summary.unfilled > 0) {
      setToast({
        type: 'error',
        msg: `还有 ${summary.unfilled} 项物资未录入实盘数量，请先录入或点击「全部实盘=系统」`,
      });
      return;
    }
    setSubmitting(true);
  ...
  const returnReason = parseReturnReason(meta.remark);
        ...
        <div className="px-4 py-3">
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <StatCell
              ...
              valueClass={summary.unfilled > 0 ? 'text-orange-600' : 'text-emerald-600'}
            />
            <StatCell
              label="差异条数"
              value={String(summary.diffCount)}
              valueClass={summary.diffCount > 0 ? 'text-red-600' : 'text-gray-700'}
            />
          </div>
