...
export default function SharePage() {
 ...
 if (!loading && !pkg) {
   return (
     <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-50 flex flex-col items-center justify-center p-8 text-center">
 ...
 const createdBy = (shareResult as any)?.template?.created_by_name || '';

 return (
   <div className="min-h-screen bg-gradient-to-b from-blue-50 to-gray-50 pb-32">
     {/* 品牌头 */}
     <header className="relative text-white">
       <div className="absolute inset-x-0 top-0 h-52 bg-gradient-to-br from-indigo-500 via-blue-600 to-blue-700 rounded-b-[32px]" />
       <div className="relative px-5 pt-12 pb-40 text-center">
         ...
         <h1 className="mt-3 text-2xl font-bold">{pkg.name}</h1>
         {createdBy && <div className="mt-1 text-sm text-white/90">由 {createdBy} 为您定制</div>}
     ...
     <main className="px-4 -mt-32 relative">
           ...
           <div key={r} className="bg-white rounded-3xl shadow-sm mb-3 overflow-hidden">
             <button onClick={() => setExpanded(s => ({ ...s, [r]: !isOpen }))}
               ...
               <div className="text-right">
                 <div className="text-xl font-bold text-blue-600">¥{disc.toFixed(0)}</div>
       ...
       <div className="bg-white rounded-3xl p-4 shadow-sm mb-4">
         ...
         <div className="mt-3 grid grid-cols-2 gap-2">
           <button onClick={() => downloadPDF()}
             className="h-11 rounded-2xl bg-blue-600 text-white text-sm font-medium shadow-sm shadow-blue-600/20">
             下载全部PDF
           </button>
       ...
       <div className="text-center text-[11px] text-gray-400 mb-4">
         如需调整方案或下单，请联系您的销售顾问：{createdBy || '销售顾问'}
       </div>
     </main>
     ...
     <div className="fixed bottom-0 left-0 right-0 p-3 pb-5 bg-gradient-to-t from-gray-50 to-transparent">
       <button onClick={() => downloadPDF('all')}
         className="w-full h-12 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2">
         📥 下载完整方案PDF
       </button>
     </div>
   </div>
