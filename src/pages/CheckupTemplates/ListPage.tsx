...
import { checkupApi, ROLES, ROLE_EMOJI, type CheckupTemplate, type Role } from './api';
...
export default function ListPage() {
  ...
  const [shareOpen, setShareOpen] = useState<{ pkg: CheckupTemplate; selected: string[] } | null>(null);
  ...
  const loadSalesUsers = async () => {
    try {
      const cfg: any = await fetch('/api/booking/config', {
  ...
  // 分享权限：管理员 或 自己创建的非公共套餐
  const canSharePkg = (pkg: CheckupTemplate) => {
    const isPublic = !!(pkg as any).is_public;
    const isOwner = !!(user?.id && (pkg as any).owner_sales_id === user.id);
    if (isAdmin) return !isPublic; // 管理员对非公共套餐可分享
    return isOwner && !isPublic;  // 销售员只能分享自己创建的非公共套餐
  };

  // 是否是别人分享给我的（不是自己创建的）
  const isSharedToMe = (pkg: CheckupTemplate) => {
    const isOwner = !!(user?.id && (pkg as any).owner_sales_id === user.id);
    return !isOwner && !(pkg as any).is_public;
  };
  ...
  const onShare = (pkg: CheckupTemplate) => {
    const currentIds: string[] = (pkg as any).cover_sales_ids || [];
    setShareOpen({ pkg, selected: [...currentIds] });
  };

  const onSaveShare = async () => {
    if (!shareOpen) return;
    try {
      const res = await checkupApi.coverSales(shareOpen.pkg.id, shareOpen.selected);
      if (res?.ok) {
        toast.success('分享成功');
        setShareOpen(null);
        load();
      } else {
        toast.error(res?.error || '分享失败');
      }
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 via-[#faf7ee] to-[#f2efe3] pb-28">
      <header className="bg-white sticky top-0 z-20 border-b border-gray-100">
        ...
        {/* 两个 Tab（feat/124: 去掉"分配给我"，合并到"我的套餐"） */}
      ...
      <main className="px-3 pt-3 space-y-3">
        ...
        {!loading && list.length === 0 ? (
          ...
          list.map(pkg => <PackageCard key={pkg.id} pkg={pkg}
            ...
            onShare={() => onShare(pkg)}
            canShare={canSharePkg(pkg)}
            isSharedToMe={isSharedToMe(pkg)}
            ownerName={(pkg as any).owner_sales_name || ''} />)
        )}
      </main>
      ...
      {shareOpen && (
        <ShareSalesModal
          ...
          selected={shareOpen.selected}
          onToggle={(uid) => setShareOpen(s => s ? {
            ...s,
            selected: s.selected.includes(uid)
              ? s.selected.filter(x => x !== uid)
              : [...s.selected, uid]
          } : s)}
          onSelectAll={() => setShareOpen(s => s ? { ...s, selected: salesUsers.map(u => u.id) } : s)}
          onClearAll={() => setShareOpen(s => s ? { ...s, selected: [] } : s)}
...
function PackageCard({ pkg, onClick, onDelete, canDelete, onEdit, canEdit, onShare, canShare, isSharedToMe, ownerName }: {
  pkg: CheckupTemplate; onClick: () => void; onDelete?: () => void; canDelete?: boolean;
  onEdit?: () => void; canEdit?: boolean; onShare?: () => void; canShare?: boolean;
  isSharedToMe?: boolean; ownerName?: string;
}) {
        ...
        <div onClick={onClick} className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[11px] text-gray-500 mt-0.5 flex items-center gap-2 flex-wrap">
              ...
              {isSharedToMe && <span className="px-1.5 py-0.5 rounded-full bg-sky-100 text-sky-700">📩 来自 {ownerName || '同事'} 分享</span>}
        ...
        <div className="flex items-center gap-2 shrink-0">
          {canShare && onShare && (
            <button onClick={(e) => { e.stopPropagation(); onShare(); }}
              className="w-8 h-8 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center border border-sky-100 active:bg-sky-100 transition"
              title="分享给销售员">
              <Users size={14} />
            </button>
          )}
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4" onClick={onCancel}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-b border-gray-100 text-xs text-gray-500 shrink-0">
          ...
          已选 <span className="font-semibold text-sky-700">{selected.length}</span> / {salesUsers.length} 人
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex gap-2">
              <button onClick={onSelectAll} className="text-xs text-sky-700 hover:underline">全选</button>
        ...
        <div className="px-5 py-3 border-t border-gray-100 shrink-0 flex justify-end gap-2">
          <button onClick={onCancel} className="px-4 py-2 text-sm rounded-xl bg-white hover:bg-gray-100 text-gray-700 border border-gray-200">取消</button>
          <button onClick={onSave} className="px-4 py-2 text-sm rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-medium">
            确认分享
          </button>
        </div>
      </div>
