...
export default function WizardFinish() {
 ...
 return (
   <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-gray-50 pb-36">
     <header className="relative text-white">
       <div className="absolute inset-x-0 top-0 h-48 bg-gradient-to-br from-emerald-500 via-emerald-600 to-teal-600 rounded-b-[32px]" />
     ...
     <main className="px-4 -mt-32 relative">
       ...
       <div className="bg-white rounded-3xl p-4 shadow-sm mb-3">
         ...
         {shareResult ? (
           ...
           <button onClick={onShare} className="w-full h-11 mt-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-medium flex items-center justify-center gap-2 shadow-md shadow-emerald-600/20">
             🔗 生成分享链接
           </button>
         )}
       </div>
         ...
         <div className="absolute left-3 right-3 top-1/2 -translate-y-1/2 bg-white rounded-3xl p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
           ...
           <div className="mt-4">
             ...
             <input type="number" value={localPrice[editRole]}
               ...
               className="w-full h-12 px-4 rounded-xl bg-gray-50 border border-gray-200 focus:border-blue-500 outline-none text-lg font-semibold"
               placeholder="请输入折后价" />
           </div>
           <div className="mt-3">
             ...
             <input type="number" min={1} max={100} step={0.1} value={localRate[editRole]}
               ...
               className="w-full h-12 px-4 rounded-xl bg-gray-50 border border-gray-200 focus:border-blue-500 outline-none text-lg font-semibold" />
             <div className="text-[11px] text-blue-700 mt-2 bg-blue-50 rounded-xl px-3 py-2">
           ...
           <div className="mt-5 grid grid-cols-2 gap-2">
             ...
             <button onClick={confirmDiscount} disabled={saving}
               className="h-11 rounded-2xl bg-blue-600 text-white font-medium disabled:opacity-60">
               {saving ? '保存中...' : '确认保存'}
             </button>
           </div>
         </div>
