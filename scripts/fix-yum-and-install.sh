#!/bin/bash
# =====================================================
# 修复 yum 仓库并安装 PostgreSQL 14 + PostgREST
# CentOS 7 专用
# =====================================================
...
echo "=== 禁用损坏的 pgdg 仓库 ==="
# 禁用所有 pgdg 仓库
for repo in $(yum repolist all 2>/dev/null | grep pgdg | awk '{print $1}'); do
    yum-config-manager --disable "$repo" 2>/dev/null || true
done

# 如果有 pgdg 的 repo 文件，也重命名
...
echo "=== 安装 PostgreSQL 14（使用 SCL 仓库） ==="
