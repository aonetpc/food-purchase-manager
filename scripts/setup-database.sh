...
rm postgrest-v12.0.0-linux-static-x64.tar.xz

echo "=== 生成 JWT Secret ==="
# 生成 256-bit secret for HS256
JWT_SECRET=$(openssl rand -base64 32)

# 生成 anon JWT token (模拟 Supabase anon key)
# 使用简单的方式生成 JWT
ANON_TOKEN=$(echo -n '{"role":"anon","alg":"HS256"}' | openssl base64 -e | tr -d '\n' | tr '+/' '-_')
